...
namespace MarioMonkeMadness.Behaviours
{
    public class VRInputProvider : SM64InputProvider
    {
        ...
        public override Vector2 GetJoystickAxes()
        {
            ...
            if (RefCache.IsSteam)
            {
                SteamVR_Action_Vector2 joystick = RefCache.Config.MoveAnalog.Value == 0 ? SteamVR_Actions.gorillaTag_LeftJoystick2DAxis : SteamVR_Actions.gorillaTag_RightJoystick2DAxis;
                joystickAxis = joystick.GetAxis(RefCache.Config.MoveAnalog.Value == 0 ? SteamVR_Input_Sources.LeftHand : SteamVR_Input_Sources.RightHand);
            }
            else
            {
                InputDevice device = RefCache.Config.MoveAnalog.Value == 0 ? ControllerInputPoller.instance.leftControllerDevice : ControllerInputPoller.instance.rightControllerDevice;
